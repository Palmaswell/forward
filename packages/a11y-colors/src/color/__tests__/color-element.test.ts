...
import { ColorProps, A11yRatio, Search } from '../types';
...
describe('Color Element', () => {
  ...
  test('Creates a new ColorElement instance', () => {
    ...
    expect(colorEl.getRGBAString(0.5)).toBe('rgba(245, 246, 250, 0.5)');
  ...
  test('Search for correspondintg AA and AAA contrast', () => {
    ...
    expect(colorEl.getAA()).toEqual(
      JSON.parse(JSON.stringify(colors.slice(aa)))
    );
    expect(colorEl.getAAA()).toEqual(
      JSON.parse(JSON.stringify(colors.slice(aaa)))
    );
  });
});
